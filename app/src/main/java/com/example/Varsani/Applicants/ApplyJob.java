...
public class ApplyJob extends AppCompatActivity {
    private TextView tv_title, tv_company, tv_location, tv_deadline, tv_description, tv_cv_filename, tv_cover_filename;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Updated: SAF picker, no permissions required
    ...
    // ✅ SAF-based picker (works Android 8 → 14+)
    private void selectFile(int requestCode) {
        Intent intent = new Intent(Intent.ACTION_OPEN_DOCUMENT);
        intent.addCategory(Intent.CATEGORY_OPENABLE);
        intent.setType("*/*"); // allows PDF, DOC, DOCX
    ...
    public void applyNow() {
        ...
        if (TextUtils.isEmpty(salary)) { showToast("Please enter expected salary"); return; }
        if (TextUtils.isEmpty(notice_period)) { showToast("Please enter your notice period"); return; }
        ...
        VolleyMultipartRequest volleyMultipartRequest = new VolleyMultipartRequest(Request.Method.POST, Urls.URL_SUBMIT_APPLICATION,
                response -> {
                    ...
                    try {
                        ...
                        if ("1".equals(status)) showSuccessDialog(msg);
                        else { showToast(msg); btn_apply_job.setVisibility(View.VISIBLE); }
            ...
            protected Map<String, DataPart> getByteData() {
                ...
                if (cvPdfUri != null) params.put("cvPdf", new DataPart(getFileName(cvPdfUri), getFileDataFromUri(cvPdfUri)));
                if (coverLetterUri != null) params.put("coverLetterPdf", new DataPart(getFileName(coverLetterUri), getFileDataFromUri(coverLetterUri)));
                return params;
            }
        };
