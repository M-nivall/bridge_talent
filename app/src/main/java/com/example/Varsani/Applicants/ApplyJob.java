...
public class ApplyJob extends AppCompatActivity {
    ...
    private TextView tv_title, tv_company, tv_location, tv_deadline, tv_description;
    private TextView tv_cv_filename, tv_cover_filename, tv_certificate_filename;

    private EditText edt_expected_salary, edt_notice_period;

    private Button btn_upload_cv, btn_upload_cover, btn_upload_certificates, btn_apply_job;

    private ProgressBar progressBar;

    private static final int PICK_COVER_LETTER_REQUEST = 1;
    private static final int PICK_CV_REQUEST = 2;
    private static final int PICK_CERTIFICATE_REQUEST = 3;

    private Uri cvPdfUri, coverLetterUri, certificateUri;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tv_certificate_filename = findViewById(R.id.tv_certificate_filename);
        ...
        btn_upload_certificates = findViewById(R.id.btn_upload_certificates);
        ...
        btn_upload_certificates.setOnClickListener(v -> selectFile(PICK_CERTIFICATE_REQUEST));

        btn_apply_job.setOnClickListener(this::alertApply);
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == RESULT_OK && data != null && data.getData() != null) {
            ...
            if (requestCode == PICK_CV_REQUEST) {
                ...
                certificateUri = selectedFileUri;
                tv_certificate_filename.setText(fileName);

                Toast.makeText(this, "Certificate Selected: " + fileName, Toast.LENGTH_SHORT).show();
            }
        }
    }

    public void alertApply(View v){
    ...
    public void applyNow() {
        ...
        if (TextUtils.isEmpty(notice_period)) {
            showToast("Please enter notice period");
            return;
        }

        VolleyMultipartRequest volleyMultipartRequest = new VolleyMultipartRequest(Request.Method.POST,
                Urls.URL_SUBMIT_APPLICATION,
            ...
            protected Map<String, DataPart> getByteData() {
                ...
                if (cvPdfUri != null) {
                    params.put("cvPdf", new DataPart(getFileName(cvPdfUri), getFileDataFromUri(cvPdfUri)));
                }

                if (coverLetterUri != null) {
                    params.put("coverLetterPdf", new DataPart(getFileName(coverLetterUri), getFileDataFromUri(coverLetterUri)));
                }

                if (certificateUri != null) {
                    params.put("certificatePdf", new DataPart(getFileName(certificateUri), getFileDataFromUri(certificateUri)));
                }
